...
> 供应商：spacexai ｜ 版本：1.0 ｜ 信息核实日期：2026-09-02
...
- **生成音频**：支持
- **备注**：xAI 官方（xai-proto video.proto）核对：视频生成模型支持文生视频、图生视频（首帧）、参考图 R2V（reference_images）、视频编辑（video 字段）与视频延长（ExtendVideo）；默认生成音频（generate_audio 默认 true），支持参考音轨（voice_id 预置音色，最多 3 条）。原榜单推断 tasks=generate+edit、inputs=reference_image，已据官方补全 extend/audio/r2v。
## 输入限制
...
> 备注：duration 1-15 秒；resolution 默认 480p，本版本支持 480p/720p（1080p 仅 grok-imagine-video-1.5 等支持 1080p 的模型提供）；aspect_ratio 默认 16:9。
