...
> 人工挑选的热度榜（20 个版本条目）——注意已不再与 `models/` 目录一一对应（目录里还有按榜单收录的骨架条目，见下文）。「分类」按能力划分：文生（文本转视频）、图生（图像转视频）、编辑（视频编辑）；一个模型可同时属于多类，并列在单元格里。最后一列是 2026-09-02 的榜单快照（AA=Elo、LMArena=Arena score，两种体系不可直接比较）。
...
> **快照说明（2026-09-02）**：AA 用 Elo，LMArena 用 Arena score（两体系不可比）；价格取 AA「用创建者 API 默认设置生成 1 分钟 1080p 视频」的口径（同模型跨榜可能不同）。排名/置信区间/样本量/发布日期/开放权重/各榜 URL 等行级明细都存在各 `models/*.json` 的 `rankings` 字段，并渲染进对应 `.md`。另有 38 个上榜但未进上述人工榜单的模型系列（如 Gemini Omni Flash、Sora 2、Veo 3、Wan 2.7、Vidu Q3…）已按系列收录进 `models/`——能力字段多为 `_missing`，待补充官方参数文档后完善。
