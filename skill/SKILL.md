...
本目录不收录第三方榜单的排名、分数、置信区间、样本量或榜单价格。历史上仅由榜单发现的 `_missing` 骨架可以保留来源说明，但不得新增 `rankings` 或其他榜单成绩字段，也不能把榜单价格写入厂商 `pricing`。用户只提供 Artificial Analysis、LMArena 等评测榜单链接时，不创建或补充模型条目；请用户改为提供厂商官方参数文档。
