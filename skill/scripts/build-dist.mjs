...
function main() {
  ...
  writeFileSync(join(ROOT, 'src/model-ids.d.ts'), modelIdTypes(catalog))
  console.log(`✓ 已生成 dist/catalog.json（${catalog.length} 条）、dist/index.json、dist/catalog.d.ts、src/model-ids.d.ts`)
}
...
function modelIdTypes(catalog) {
  const ids = catalog.map((entry) => entry.model_id)
  const union = ids.map((id) => `  | '${id}'`).join('\n')
  return `// Generated by build-dist.mjs; do not edit by hand.
export type ModelId =
${union}
`
}
